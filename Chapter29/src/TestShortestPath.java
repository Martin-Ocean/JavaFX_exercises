...
public class TestShortestPath {
    public static void main(String[] args) {
        String[] vertices = {"Valparaiso", "Lafayette", "Fort Wayne", "Kokomo", "Indianapolis"};

        int[][] edges = {
                {0, 1, 90}, {0, 2, 109},
                {1, 0, 90}, {1, 3, 44}, {1, 4, 62},
                {2, 0, 109}, {2, 3, 88},
                {3, 2, 88}, {3, 1, 44}, {3, 4, 59},
                {4, 3, 59}, {4, 1, 62}
        };
        ...
        WeightedGraph<String>.ShortestPathTree tree1 =
                graph1.getShortestPath(graph1.getIndex("Lafayette"));
        tree1.printAllPaths();

        // Display shortest paths from Houston to Chicago
        System.out.print("Shortest path from Lafayette to Indy: ");
        java.util.List<String> path = tree1.getPath(4);
        for (String s : path) {
            System.out.print(s + " ");
        }
    }

    public abstract static class AbstractGraph<V> implements Graph<V> {
    ...
    public static class WeightedEdge extends AbstractGraph.Edge
