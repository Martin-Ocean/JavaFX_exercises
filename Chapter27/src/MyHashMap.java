...
public class MyHashMap<K, V> implements MyMap<K, V> {
  ...
  public boolean containsValue(V value) {
    table.forEach(item -> item.getValue().equals(value));
    return false;
  }
  ...
  public java.util.Set<Entry<K,V>> entrySet() {
    ...
    //table.forEach(item -> set.add(item));
    return set;
  }
  ...
  public V get(K key) {
    ...
    while(true) {
      if (table.get(bucketIndex) != null) {
        if (table.get(bucketIndex).getKey().equals(key)) return table.get(bucketIndex).getValue();
      } else return null;
      bucketIndex++;
  ...
  public java.util.Set<K> keySet() {
    ...
    table.forEach(item -> {
      if (item != null) set.add(item.getKey());
    });
    return set;
  }
  ...
  public void remove(K key) {
    ...
    while (table != null) {
      if (table.get(bucketIndex).getKey().equals(key)) {
        table.remove(bucketIndex);
        size--;
        break;
      }
      bucketIndex++;
  ...
  public java.util.Set<V> values() {
    ...
    table.forEach(item-> set.add(item.getValue()));
    return set;
  }
  ...
  private void rehash() {
    ...
    //table.forEach(item->table.add(null));
    for (int i = 0; i < capacity; i++)
      table.add(null);
